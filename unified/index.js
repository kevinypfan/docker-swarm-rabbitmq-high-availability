...
const createSimpleLogger = (component) => {
  ...
  return {
    info: (msg, data = {}) => console.log(`[INFO] ${component} - ${msg}${formatData(data)}`),
    error: (msg, data = {}) => console.error(`[ERROR] ${component} - ${msg}${formatData(data)}`),
    warn: (msg, data = {}) => console.warn(`[WARN] ${component} - ${msg}${formatData(data)}`),
    debug: (msg, data = {}) => isDev && console.log(`[DEBUG] ${component} - ${msg}${formatData(data)}`)
  };
};
...
class RabbitMQClient {
  constructor() {
    ...
    this.clientId = process.env.HOSTNAME || `client-${Math.random().toString(36).substr(2, 9)}`;
  ...
  async setupInfrastructure() {
    try {
      ...
      this.logger.error('Failed to setup infrastructure', { error: error.message });
      throw error;
    }
  }
